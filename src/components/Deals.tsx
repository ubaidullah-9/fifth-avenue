...
import { Ticket, Flame, Check } from 'lucide-react';
      ...
      <div className="border-2 border-dashed border-[#FFB800]/30 group-hover:border-[#FFB800]/60 rounded-xl h-full flex flex-col relative z-10 transition-colors duration-300 overflow-hidden bg-stone-950">
        ...
        <div className={`p-6 flex flex-col flex-grow ${deal.image ? 'pt-4' : ''}`}>
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
            className={`w-full py-3.5 font-bold rounded-lg transition-colors shadow-[0_0_15px_rgba(255,184,0,0.3)] hover:shadow-[0_0_25px_rgba(255,184,0,0.5)] mt-auto flex items-center justify-center gap-2 ${added ? 'bg-green-500 text-white' : 'bg-[#FFB800] hover:bg-[#e5a600] text-stone-950'}`}
          >
            {added ? <><Check className="w-5 h-5" /> Added to Order</> : 'Claim Deal'}
          </motion.button>
        </div>
      </div>
    </motion.div>
