...
export default function AdminDashboard() {
        ...
        if (u.email?.toLowerCase() === 'ubaidullah.bhatti99@gmail.com') {
          ...
          try {
            ...
            if (adminDoc.exists()) {
              ...
              const profileDoc = await getDoc(doc(db, 'admin_profiles', u.uid));
              setIsAdmin(profileDoc.exists());
            }
          } catch (e) {
            setIsAdmin(false);
          }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Auto-create an admin profile if they are logging in for the first time
      // The rules allow creation if request.auth.uid == userId
      try {
        const profileRef = doc(db, 'admin_profiles', result.user.uid);
        const profileSnap = await getDoc(profileRef);
        if (!profileSnap.exists() && result.user.email) {
        ...
        console.log('Could not auto-create profile, continuing login...', err);
      }
    } catch (e: any) {
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-stone-950 p-4">
        <BrandLogo className="w-48 mb-6" />
        <div className="bg-stone-900 p-8 rounded-2xl border border-stone-800 text-center max-w-md w-full">
          <h2 className="text-2xl font-bold text-white mb-2">Admin / Staff Login</h2>
          <p className="text-stone-400 mb-6">Sign in with your Google account to access the dashboard. Staff accounts will be automatically created upon first login.</p>
          
          <button 
            onClick={handleLogin}
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-stone-900 font-bold py-3 px-4 rounded-xl transition-colors mb-4"
          >
          ...
          {loginError && (
            <div className="mt-4 bg-red-500/10 border border-red-500/50 text-red-500 p-4 rounded-lg text-sm text-left">
              {loginError}
            </div>
          )}
    ...
    <div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-stone-900 p-6 rounded-xl border border-stone-800 mb-8 grid grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm mb-1 text-stone-400">Image URL</label><input name="image" defaultValue={editing.image} className="w-full bg-stone-950 border border-stone-800 rounded p-2" placeholder="https://..." /></div>
